...
private:
    ...
    // 日志缓冲（GameController 不直接输出，只暂存字符串供 UI 取走）
    ...
    void Tick_Idle() {
        ...
        // 每次 Tick 只处理一个 ready actor，保证 UI 能逐帧显示（敌方行动不会瞬间完成）
        ...
        if (decision == 0 || currentActor->GetSkills().empty()) {
            ...
            if (!availableSkills.empty()) {
                ...
                if (!skill->NeedTarget() || (target && target->IsAlive())) {
                    std::string msg = ">> " + currentActor->GetName() + " 使用 " + skill->GetName();
            ...
            PushLog(">> " + currentActor->GetName() + " 休息恢复气力！");
        }
...
public:
    ...
    void SubmitSkill(int skillIndex) {
        ...
        if (skillIndex == -1) {
            currentPhase = Phase::PlayerMainMenu; // 返回主菜单
            return;
        }
        ...
        if (!chosenSkillPtr->IsAvailable(currentActor)) {
            PushLog("技能不可用！（气力不足或其他条件不满足）");
            return; // 保持在技能菜单，等待重新选择
        }
